...
fn main() {
    ...
    let lines: Vec<Vec<u32>> = input.split("\n").map(|l| { l.chars().map(|n| n.to_digit(10).unwrap()).collect() }).collect();
    ...
    for (i, line) in lines.iter().enumerate() {
        for (j, number) in line.iter().enumerate() {
            ...
            let next = if j < line.len() - 1 { &line[j + 1] } else { &10 };
    ...
    for point in horizontal_lows.iter() {
        let above = if point.0 > 0 { lines[point.0 - 1][point.1] } else { 10 };
        let below = if point.0 < lines.len() - 1 { lines[point.0 + 1][point.1] } else { 10 };
    ...
    println!("Product of three largest basins: {}", basin_sizes[0] * basin_sizes[1] * basin_sizes[2]);
}

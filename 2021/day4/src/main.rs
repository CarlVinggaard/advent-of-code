...
fn parse_board(b: &str) -> Board {
    ...
    let board_vec: Vec<[u32; 5]> = b
        .split("\n")
        .map(|row| -> [u32; 5] {
            let mut parsed = [0; 5];

            let row_vec: Vec<u32> = row
                .split(" ")
                .map(|c| c.trim())
                .filter(|c| *c != "")
...
fn main() {
    ...
    let numbers_input =
        fs::read_to_string(numbers_filename).expect("There was a problem reading the file");
    let boards_input =
        fs::read_to_string(boards_filename).expect("There was a problem reading the file");

    let numbers: Vec<u32> = numbers_input
        .split(",")
    ...
    for number in numbers {
        ...
        for board in &mut boards {
            ...
            if board.has_won(&drawn) {
                ...
                println!(
                    "A board has won! Product of sum ({}) and drawn number ({}) is {}",
                    sum, number, product
                );
            }
        }

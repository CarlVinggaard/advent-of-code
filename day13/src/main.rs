...
impl Paper {
    fn fold(&mut self, direction: &Direction, axis: &usize) {
        let mut dots: Vec<(usize, usize)> = self.dots.clone().into_iter().filter(|d| {
            match &direction {
...
fn main() {
    ...
    let instruction_input = fs::read_to_string("./instructions.txt").expect("There was an error reading the file");

    let dots: Vec<(usize, usize)> = input.lines().map(|l| {
        let (x_str, y_str) = l.split_once(",").unwrap();
        (x_str.parse::<usize>().unwrap(), y_str.parse::<usize>().unwrap())
    }).collect();

    let instructions: Vec<(Direction, usize)> = instruction_input.lines().map(|l| {
        let words: Vec<&str> = l.split_whitespace().collect();
        let (dir_str, axis_str) = words[2].split_once("=").unwrap();

        let dir = if dir_str == "x" { Direction::X } else { Direction::Y };
        let axis = axis_str.parse::<usize>().unwrap();

        (dir, axis)
    }).collect();
    
    let mut paper = Paper { dots, x_length: 10, y_length: 14 };
